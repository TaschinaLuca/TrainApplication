    ...
    public List<List<RouteLeg>> findRoutes(int startStationId, int endStationId) {
        ...
        for (Route r : allRoutes) {
            ...
            for (RouteStop rs : stops) {
                if (rs.station().id() == startStationId) start = rs;
                if (rs.station().id() == endStationId) end = rs;
            }
            if (start != null && end != null && start.stopOrder() < end.stopOrder()) {
                results.add(List.of(new RouteLeg(r, start.station(), end.station(), start.departureTime(), end.arrivalTime())));
            }
        }
        ...
        for (Route r1 : allRoutes) {
            ...
            if (start1 == null) continue;

            for (RouteStop end1 : stops1) {
                if (end1.stopOrder() <= start1.stopOrder()) continue;
                // Possible changeover station: end1.station()
                for (Route r2 : allRoutes) {
                    if (r1.id() == r2.id()) continue;
                    List<RouteStop> stops2 = routeStopsMap.get(r2.id());
                    RouteStop start2 = stops2.stream().filter(s -> s.station().id() == end1.station().id()).findFirst().orElse(null);
                    if (start2 == null) continue;
                    
                    // Simple time check: start2 departure must be >= end1 arrival (lexicographically works for YYYY-MM-DD HH:MM)
                    if (start2.departureTime().compareTo(end1.arrivalTime()) >= 0) {
                        RouteStop finalEnd = stops2.stream().filter(s -> s.station().id() == endStationId && s.stopOrder() > start2.stopOrder()).findFirst().orElse(null);
                        if (finalEnd != null) {
                            results.add(List.of(
                                new RouteLeg(r1, start1.station(), end1.station(), start1.departureTime(), end1.arrivalTime()),
                                new RouteLeg(r2, start2.station(), finalEnd.station(), start2.departureTime(), finalEnd.arrivalTime())
                            ));
                        }
                    }
                }
